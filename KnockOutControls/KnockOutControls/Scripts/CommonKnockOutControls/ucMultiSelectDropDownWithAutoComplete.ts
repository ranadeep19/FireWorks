...
class MultiSelectDropDownWithAutoCompleteViewModel {

    private data: any;
    private mappedData: any;
    private selectedCheckBoxes: any;
    private _selectedCheckBoxes: any;//for internal use
    private toolTip: KnockoutComputed<any>;
    private formattedSelectedCheckBoxes: KnockoutObservable<any>;
    private selectSingle: boolean;
    private addAllOption: boolean;
    private searchString: KnockoutObservable<any>;
    private defaultSelection: any;
    private parentControlId: KnockoutObservable<string>;
    private currentStartCount: any;
    private currentEndCount: any;    
    private dataBindCount: any;
    constructor(params) {
        ...
        this.data = [];
        this.mappedData = [];
        this.formattedSelectedCheckBoxes = ko.observable<any>();
        ...
        ko.computed({
            ...
            function () {
                ...
                if (searchString.charCodeAt(0) == 160)  //First space character in contenteditable div is always &nbsp;. So replacing with space(" ")
    ...
    private BindNextSetOfData(count, searchString = '') {
                var lis = '';
        var _tempCount = 0;
        ...
        if (this.addAllOption == true && searchString == "" && this.data.length > 0 && i <= 0) {
                    lis += '  <li class="k-item" >'
            + '<input type="checkbox" value="' + this.data[0].value + ((this.data[0].checked == true) ? '" checked' : '" ') + '/>'
                    + '<span>' + this.data[0].text + '</span></li>';
            _tempCount++;
            i++;
                }
        ...
        for (i; i < this.data.length && _tempCount < count + extra; i++) {
            if (this.data[i].text.toLowerCase().indexOf(searchString.toLowerCase()) > -1 && this.data[i].text != "All") {
                        lis += '  <li class="k-item" >'
                            + '<input type="checkbox" value="' + this.data[i].value + ((this.data[i].checked == true) ? '" checked' : '" ') + '/>'
                        + '<span>' + this.data[i].text + '</span></li>';
                _tempCount++;
                    }
                }
        ...
        if (_tempCount < count + extra) {
            for (var j = this.currentEndCount - 1; j >= 0 && _tempCount < count + extra; j--) {
                if (this.data[j].text.toLowerCase().indexOf(searchString.toLowerCase()) > -1 && this.data[j].text != "All") {
                    lis = '  <li class="k-item" >'
                    + '<input type="checkbox" value="' + this.data[j].value + ((this.data[j].checked == true) ? '" checked' : '" ') + '/>'
                    + '<span>' + this.data[j].text + '</span></li>' + lis;
                    _tempCount++;
    ...
    private BindPrevSetOfData(count, searchString = '') {
        var lis = '';
        var _tempCount = 0;
        var extra = 1;
        ...
        for (--i; i >= 0 && _tempCount < count + extra; i--) {
            if (this.data[i].text.toLowerCase().indexOf(searchString.toLowerCase()) > -1 && this.data[i].text != "All") {
                lis = '  <li class="k-item" >'
                + '<input type="checkbox" value="' + this.data[i].value + ((this.data[i].checked == true) ? '" checked' : '" ') + '/>'
                + '<span>' + this.data[i].text + '</span></li>' + lis;
                _tempCount++;
            }
        }
        ...
        if (this.addAllOption == true && searchString == "" && this.data.length > 0 && i <= 0) {
            lis = '  <li class="k-item" >'
            + '<input type="checkbox" value="' + this.data[0].value + ((this.data[0].checked == true) ? '" checked' : '" ') + '/>'
            + '<span>' + this.data[0].text + '</span></li>' + lis;
            _tempCount++;
        }
    ...
    private CheckUncheckAllOption(target) {
        if ($(target).val() == 'All') {
    ...
    private CheckUncheckBindedData(value, checked) {
    ...
    private CheckBoxChecked(data, event) {
        ...
        if (value == 'All')
            ...
            if (this.selectSingle == true) {
                this.CheckUncheckBindedData($.map(this.data, function (item) { return item.checked == true && item.value != value ? item.value : null; })[0], false);
                $(event.target).closest("ul").find("[type=checkbox]:checked").prop('checked', false);
                $(event.target).prop('checked', true);
            }
            else if (this.addAllOption) {
                if ($.map(this.data, function (item) { return item.checked == true ? item.checked : null; }).length == this.data.length - 1) {
    ...
    private BindData(newData) {
        if (newData != undefined) {
            ...
            this.mappedData = $.map(this.data, function (item) { return item.value.toString(); });
            this.searchString(" ");
            this.searchString("");
    ...
    private scrolled(data, event) {
        ...
        if ((elem.scrollTop + elem.offsetHeight) > elem.scrollHeight) {
            ...
            if (isDataPresent == true) elem.scrollTop = 5;
        }
    ...
    private dropDownClick(data, event) {
    ...
    private UpdatePageWithSelection(fireStatusChanged: boolean , event : any = null) {
        var array = [];
        var arrayText = [];
    ...
    private CancelClick(data, event) {
...
ko.components.register('multi-select-with-auto-complete', {
    ...
    template:
    ' <div class="multiSelectDefaultDimensions" data-bind="parentControlID :parentControlId">\
